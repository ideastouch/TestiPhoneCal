...
-(void)drawDateWords{
	...
	for(int i=1;i<dayCount+1;i++)
	{
		...
		[date drawAtPoint:CGPointMake(x*s_width+15,y*itemHeight+headHeight+itemHeight/7*2) withFont:weekfont];
...
- (void) drawToday{
	...
	if(today.month==currentMonthDate.month && today.year==currentMonthDate.year)
	{
		...
		[date drawAtPoint:CGPointMake(x*swidth+15,y*itemHeight+headHeight+itemHeight/7*2) withFont:weekfont];
